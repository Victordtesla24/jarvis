...
// VideoFeed/MediaPipe removed — no webcam/gesture (system-monitor dashboard)
...
const App: React.FC = () => {
              ...
              {bootStep >= 1 && (
                  <div className="z-10 flex flex-col items-center gap-4 w-96">
                      <div className="text-2xl font-display font-bold animate-pulse">
                          {bootStep === 1 && "System booting..."}
                          {bootStep === 2 && "Loading neural network..."}
                      ...
                      <div className="text-xs text-gray-500 h-20 overflow-hidden w-full text-center leading-tight">
                          {bootStep >= 1 && <div> Memory allocation check... OK</div>}
                          {bootStep >= 1 && <div> GPU delegation... Assigned</div>}
                          {bootStep >= 2 && <div> Loading MEDIA_PIPE.WASM...</div>}
                          {bootStep >= 2 && <div> Connecting satellite uplink...</div>}
                          {bootStep >= 3 && <div> Retinal scan... Bypassed</div>}
  ...
  return (
    <div className="relative w-full h-screen bg-black overflow-hidden animate-flash">
      {/* 1. Background Camera Layer — REMOVED (no webcam/gesture; dashboard is system-monitor, not gesture demo) */}
