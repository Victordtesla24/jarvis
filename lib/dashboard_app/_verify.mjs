...
// (e) Gesture debug overlay — proves the camera→ref pipeline is observable.
// Hidden by default; appears with ?debug. Headless has no camera so values
// stay at no/0.00, but the overlay rendering at all confirms it polls the
// SAME handTrackingRef the reactor + globe consume.
let debugOverlay = { ok: false, hiddenByDefault: false, text: '' };
{
  const hidden = await page.$('[data-testid="gesture-debug-overlay"]');
  debugOverlay.hiddenByDefault = hidden === null;
  await page.goto(`${BASE}/?skipboot&debug`, { waitUntil: 'domcontentloaded' });
  await page.waitForSelector('canvas', { timeout: 15000 });
  const el = await page.waitForSelector('[data-testid="gesture-debug-overlay"]', { timeout: 8000 }).catch(() => null);
  if (el) {
    const text = await el.innerText();
    debugOverlay.ok = /GESTURE DEBUG/.test(text) && /L hand/.test(text) && /rotX/.test(text) && /expand/.test(text) && /pinch/.test(text);
    debugOverlay.text = text.replace(/\n/g, ' | ');
  }
  await sleep(800);
  await page.screenshot({ path: `${OUT}/_verify_debug_overlay.png` });
}
...
console.log('debug overlay hidden by default:', debugOverlay.hiddenByDefault);
console.log('debug overlay renders with ?debug:', debugOverlay.ok, '|', debugOverlay.text);
