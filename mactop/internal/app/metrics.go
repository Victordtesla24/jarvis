...
func startPrometheusServer(port string) {
	...
	registry.MustRegister(customDVHOPCPUPct)
	registry.MustRegister(customGUMERMBs)
	registry.MustRegister(customCCTCDelta)
...
func CollectGUMER(chipLabel string, prevSwapBytesVal, currSwapBytes int64, intervalSec float64, gpuActivePct float64) {
	if gpuActivePct < 15.0 {
		customGUMERMBs.WithLabelValues(chipLabel).Set(0)
		return
	}
	deltaBytes := currSwapBytes - prevSwapBytesVal
	if deltaBytes < 0 {
		deltaBytes = 0
	}
